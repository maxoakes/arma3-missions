...
if (_set) then 
{
	_pos = [getPos _loc, 0, locationTeleportArea, 5, 0, maxGradient, 0] call BIS_fnc_findSafePos;
...
_start = createVehicle [centerObject, _pos, [], 0, "CAN_COLLIDE"];
...
_markerManager = [_pos,_randomName,_thePlayer] spawn
{
	...
	while {(alive (_this select 2)) and ((_this select 2) distance2D (getMarkerPos "respawn_west") > 10)} do
	{
...
_start addAction ["<t color='#ff0000'>I am done. Go back to base.","removeAllActions (_this select 0);"];
...
_counter = _thePlayer spawn
{
	...
	"
		_home = getPos (nearestObjects [(_this select 1),[centerObject],maxSpawnDistance] select 0);
...
if (alive _thePlayer) then {_thePlayer setPos [(getPos respawnPad select 0),(getPos respawnPad select 1),(getPos respawnPad select 2)+spawnHeightAboveSeaLevel];};